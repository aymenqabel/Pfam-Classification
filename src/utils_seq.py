...
def create_dataset(data, max_nb_chars=21,max_sequence_length=2975):
    ...
    query_train = transform_to_dataset(tokenizer=train_tokenizer, text = text_train,names=names_train, y = y_train, max_sequence_length=max_sequence_length)
    query_val = transform_to_dataset(tokenizer=train_tokenizer, text = text_val, y = y_val,names=names_val, max_sequence_length=max_sequence_length)
    query_test = transform_to_dataset(tokenizer=train_tokenizer, text = text_test, y = y_test,names=names_test, max_sequence_length=max_sequence_length)
