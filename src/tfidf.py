...
def main(args):
    """Main function to train the Random forest model on the tfidf vectors extracted from the protein sequences

    Args:
        args (): arguments to use for the models
    """
    # Load data
    data = {}
    ...
    cls = Pipeline([('tfidf', TfidfVectorizer(min_df=args.min_tf, max_features=args.max_f,
                                              analyzer='char_wb', 
                                              ngram_range=(args.ngram,args.ngram))),
                        ('rf', RandomForestClassifier(verbose = args.verbose*1, 
